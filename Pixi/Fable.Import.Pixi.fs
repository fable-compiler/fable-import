module rec Fable.Import.Pixi
open System
open System.Text.RegularExpressions
open Fable.Core
open Fable.Import.JS

type [<AllowNullLiteral>] [<Import("*","Application")>] Application(?width: float, ?height: float, ?options: ApplicationOptions, ?noWebGL: bool, ?sharedTicker: bool, ?sharedLoader: bool) =
...
and [<AllowNullLiteral>] [<Import("*","PointLike")>] PointLike() =
    member __.x with get(): float = jsNative and set(v: float): unit = jsNative
    member __.y with get(): float = jsNative and set(v: float): unit = jsNative
...
and [<AllowNullLiteral>] ApplicationOptions =
...
and [<AllowNullLiteral>] DefaultRendererPlugins =
...
and [<AllowNullLiteral>] UniformDataMap<'U> =
    obj
...
and [<AllowNullLiteral>] SpriteMaskFilterUniforms =
    obj
...
module extras =
    ...
    and [<AllowNullLiteral>] [<Import("AnimatedSprite","extras")>] AnimatedSprite(textures: U2<ResizeArray<Texture>, ResizeArray<AnimatedSpriteTextureTimeObject>>, ?autoUpdate: bool) =
        ...
        member __.onLoop with get(): Func<unit, unit> = jsNative and set(v: Func<unit, unit>): unit = jsNative
...
module interaction =
    ...
    and [<AllowNullLiteral>] [<StringEnum>] InteractionPixiEvents =
        | Added | Removed
...
module core =
    ...
    and [<AllowNullLiteral>] AbstractFilter<'U> =
        Filter<'U>
